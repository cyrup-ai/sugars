...
pub mod sync {
    ...
    impl<V> ObjectIdMap<V> {
        ...
        pub fn insert(&self, key: gix_hash::ObjectId, value: V) -> Option<V> {
            self.shards[key.as_slice()[0] as usize]
                .lock()
                .insert(key, value)
        }
    }
...
/// Fast hashing utilities for Git objects.
///
/// This module provides optimized hashers for keys that are already robust hashes
/// (like Git ObjectIds), avoiding redundant hash computation.
pub mod hash {
